...
class SiteSettingsViewTest(TestCase):
    def setUp(self):
        ...
        call_command('createsettings', '--no-color', stdout=out,)
        User = get_user_model()
        self.super_user = User.objects.create_user(TEST_USER_EMAIL,
                                                   TEST_USER_PASSWORD,
                                                   is_staff=True)
        self.standard_user = User.objects.create_user(TEST_USER_EMAIL_2,
